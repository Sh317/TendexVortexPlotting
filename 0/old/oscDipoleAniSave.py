...
camera_view = dict(
    ...
    center=dict(x=0, y=0, z=0),     # Focus point
...
for i, frame in enumerate(fig.frames):
    fig_frame = go.Figure(data = frame.data,
        layout=go.Layout(
            title='Time: %s' % time_steps[i],
            scene=dict(
                xaxis_title="x",
                yaxis_title="y",
                zaxis_title="z",
    ...
    #fig_frame.update_layout(scene=dict(aspectmode='cube'))  # Optional for cube-like aspect ratio
    fig_frame.write_image("frames/f_%s.png" % i)

# Create a GIF from saved frames
with imageio.get_writer("seeds=%s_its=%s_d=%s.gif" %(seed_num, num_its, distance), mode="I", duration=time_per_frame/50) as writer:
    for i in range(len(frames)):
        filename = "frames/f_%s.png" % i
