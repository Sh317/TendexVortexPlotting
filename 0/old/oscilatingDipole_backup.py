...
def f(r_V): #Assuming off diagonal = 0, assuming I33 = 0
    ...
    E = (-6 * E_1 / (r**5)) + 60 * E_2 / (r**7) + (15 * E_3 / (r**7)) + (-105 * E_4 / (r**9))
...
def e_solve(E, past_e_vecs, past_e_vals):
    ...
    for i in range(3):
        for j in range(3):
            ...
            if e_vals[j] != 0 and past_e_vals[i] != 0:
                if (np.sign(past_e_vals[i]) == icity and np.sign(e_vals[j]) == icity): #/or (dot < 0 and np.sign(past_e_vals[i]) != icity and np.sign(e_vals[j]) != icity)):
                    dots[i, j] = np.abs(dot)
        ...
        r_change = past_e_vecs[:, index]# * past_e_vals[index // 3]
