...
lim = 150 # x y bounds for plot
delta_0 = 10**(-2) # Min deviation from next step
...
test_ax.scatter(x0, y0)
test_fig.savefig('./plots/seeds.png', dpi=300)
    ...
    for t in [.5*np.pi]:
        for i in range(seeds):
            for icity in [1]:
                ...
                vect_c = rka_iter(t, sepX, sepY, x, y, z, num_its, icity, ending_tolerance, delta_0, safety, h0)
                its = vect_c.its
                x = vect_c.x[0:its]
                y = vect_c.y[0:its]
                z = vect_c.z[0:its]
        ...
        title = 'pi* ' + str(int(((t) / np.pi)*100)/100) +' S:' + str(sepX)
        print(title)
        pos_fig.suptitle(title)


        pos_fig.savefig('./plots/' + str(n) + '(' + title + ')pos_double.png', dpi=300)
        #plt.show()
        pos_ax.cla()
        n += 1
        #neg_fig.savefig('./plots/(' + str(int(t*10) / 10) + ')NoColor180Dneg_double_' + title + '.png', dpi=300)